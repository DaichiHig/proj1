...
int main(){
	...
	while(1){
		...
		//入力のエラー処理むずすぎ
		if(mode == 1) {//入力情報の表示
			t = cliadd;//１つ目の切り抜くデータ
			p = strchr(t, '&');
			*p = '\0';//文字列を区切る
			t = strchr(t, '=')+1;
			if(t != p){//addrが空欄ではないとき
				fprintf(istream, "Your address is %s.<br>", t);
			}
			
			fprintf(istream, "OK1.<br>");
			
			t = p + 1;//次のデータ
			p = strchr(t,'&');
			*p = '\0';//文字列を区切る
			t = strchr(t, '=')+1;
			if(t != p){//nameが空欄ではないとき
				fprintf(istream, "Your name is %s.<br>", t);
			}
			
			fprintf(istream, "OK2.<br>");
			...
			*p = '\0';//文字列を区切る
			...
			if(t != p){//=の次が空欄ではないとき
				if(strcmp(t, "on") == 0){//
					fprintf(istream, "Your are fine.<br>");
					t = p + 1;//次のデータ
					t = strchr(t, '=')+1;
				}else{
					fprintf(istream, "Your are not fine.<br>");
					t = strchr(t, '=')+1;
				}
			}
			
			fprintf(istream, "OK3.<br>");
			
			if(t != p){
				fprintf(istream, "Your sex is %s.<br>", t);
			}
			
			fprintf(istream, "OK4.<br>");
		}
